...
export function Navigation() {
  ...
  // Reusable objects for useFrame to prevent garbage collection
  const moveRef = useRef(new THREE.Vector3(0, 0, 0))
  ...
  useFrame((state, delta) => {
    ...
    move.applyAxisAngle(yAxisRef.current, rotation.current.yaw)
    ...
    // Apply rotation
    eulerRef.current.set(rotation.current.pitch, rotation.current.yaw, 0, 'YXZ')
